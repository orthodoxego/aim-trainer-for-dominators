...
class Font:
    ...
    def __init__(self, engine):
        ...
        self.fire_text = ""
        self.reaction_text = ""
        self.damage_render = None
        self.damage_render_black = None
    ...
    def add_moving_text(self, stroke, damage):
    ...
    def act(self, delta_time):
        for moving_text in self.moving_text_lists:
            ...
            if moving_text.y < 50:
                self.engine.adding_damage(moving_text.damage)
    ...
    """
    def draw(self, scene, damage, count_firing, reaction_time):
        damage_text =   f"    Урон: {damage}"
        fire_text =     f"Цель/Уб.: {count_firing}/{damage // 100}"
        ...
        if self.damage_text != damage_text:
            self.damage_render = self.font.render(damage_text, False, (255, 128, 0))
            self.damage_render_black = self.font.render(damage_text, False, (20, 0, 0))
        ...
        scene.blit(self.damage_render_black, (Settings.WIDTH - 353, 47))
        scene.blit(self.damage_render, (Settings.WIDTH - 353, 45))
